...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 10 14:56:26 2023
...
class AdminRegistration(tk.Frame):
    ...
    def __init__(self, master,user,pswd):
        ...
        canvas = tk.Canvas(self.frame, width=500,height=500)
        ...
        label_adminfname = ttk.Label(self.frame, text="First Name:")
        label_adminfname.grid(column=0, row=1, padx=10, pady=5, sticky=tk.W)
        self.entry_adminfname = ttk.Entry(self.frame)
        self.entry_adminfname.grid(column=1, row=1, padx=10, pady=5, sticky=tk.W)

        label_adminlname = ttk.Label(self.frame, text="Last Name:")
        label_adminlname.grid(column=0, row=2, padx=10, pady=5, sticky=tk.W)
        self.entry_adminlname = ttk.Entry(self.frame)
        self.entry_adminlname.grid(column=1, row=2, padx=10, pady=5, sticky=tk.W)

        label_username = ttk.Label(self.frame, text="Username:")
        label_username.grid(column=0, row=3, padx=10, pady=5, sticky=tk.W)
        self.entry_adminusername = ttk.Entry(self.frame)
        self.entry_adminusername.grid(column=1, row=3, padx=10, pady=5, sticky=tk.W)

        label_adminpassword = ttk.Label(self.frame, text="Password:")
        label_adminpassword.grid(column=0, row=4, padx=10, pady=5, sticky=tk.W)
        self.entry_adminpassword = ttk.Entry(self.frame, show="*")
        self.entry_adminpassword.grid(column=1, row=4, padx=10, pady=5, sticky=tk.W)


        label_confirm_adminpassword = ttk.Label(self.frame, text="Confirm Password:")
        label_confirm_adminpassword.grid(column=0, row=5, padx=10, pady=5, sticky=tk.W)
        self.entry_confirm_adminpassword = ttk.Entry(self.frame, show="*")
        self.entry_confirm_adminpassword.grid(column=1, row=5, padx=10, pady=5, sticky=tk.W)

        

        label_state = ttk.Label(self.frame, text="State:")
        label_state.grid(column=0, row=10, padx=10, pady=5, sticky=tk.W)
        ...
        self.combobox_state.grid(column=1, row=10, padx=10, pady=5, sticky=tk.W)
        ...
        label_street.grid(column=0, row=12, padx=10, pady=5, sticky=tk.W)
        self.entry_street = ttk.Entry(self.frame)
        self.entry_street.grid(column=1, row=12, padx=10, pady=5, sticky=tk.W)

        label_zip_code = ttk.Label(self.frame, text="Zip Code:")
        label_zip_code.grid(column=0, row=13, padx=10, pady=5, sticky=tk.W)
        self.entry_zip_code = ttk.Entry(self.frame)
        self.entry_zip_code.grid(column=1, row=13, padx=10, pady=5, sticky=tk.W)
    
        label_phone = ttk.Label(self.frame, text="Phone:")
        label_phone.grid(column=0, row=14, padx=10, pady=5, sticky=tk.W)
        self.entry_phone = ttk.Entry(self.frame)
        self.entry_phone.grid(column=1, row=14, padx=10, pady=5, sticky=tk.W)
    
        label_email = ttk.Label(self.frame, text="Email:")
        label_email.grid(column=0, row=15, padx=10, pady=5, sticky=tk.W)
        self.entry_email = ttk.Entry(self.frame)
        self.entry_email.grid(column=1, row=15, padx=10, pady=5, sticky=tk.W)
    ...
    def submit(self):
        # get the form data
        adminfname = self.entry_adminfname.get()
        adminlname = self.entry_adminlname.get()
        adminusername = self.entry_adminusername.get()
        adminpassword = self.entry_adminpassword.get()
        confirm_adminpassword = self.entry_confirm_adminpassword.get()
        street = self.entry_street.get()
        city = self.combobox_city.get()
        state = self.combobox_state.get()
        zip_code = self.entry_zip_code.get()
        adminphone = self.entry_phone.get()
        adminemail = self.entry_email.get()

        # check if the password and confirm password match
        if adminpassword != confirm_adminpassword:
        ...
        try:
            ...
            mycursor.execute("select check_registration(%s)", (adminusername,))
            ...
            # execute the query to retrieve the bloodbank_name values
            mycursor.execute("SELECT bloodbank_name FROM bloodbank")
            
            # fetch all the bloodbank_name values
            result = mycursor.fetchall()
            
            # randomly select one name from the result set
            bloodbankname = random.choice(result)[0]

        
        
            # insert the data into the database
        
            sql = "INSERT INTO admin (admin_fname, admin_lname, admin_username, admin_password, street, city, state, zip_code, admin_phone, admin_email, bloodbank_name) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
       
        
            mycursor.execute(sql, (adminfname, adminlname, adminusername, adminpassword, street, city, state, zip_code, adminphone, adminemail, bloodbankname))
            self.db.commit()
